<#
.SYNOPSIS
    Get-ADObjectLastLogon gets the last logon timestamp of an Active Directory user.
...
.EXAMPLE
    Get-ADObjectLastLogon.ps1 -samAccountName s.stollane

    Accepts a single user SAM account name

.EXAMPLE
    Get-ADObjectLastLogon.ps1 -samAccountName "b.simpson","p.griffin"

    Accepts an array of user SAM account names

.EXAMPLE
    Get-AdUser -Filter 'Enabled -eq $true' | Get-ADObjectLastLogon.ps1
...
(
    ...
    [Alias("UserName", "User", "Computer", "ComputerName")]
...
process {
    ...
    foreach ($object in $samAccountName) {
    
        $resultlogon = @()

        # Get user to test for existence
        $dn = Get-AdObject -Filter 'samAccountName -eq $object' -Properties distinguishedName | Select-Object -ExpandProperty distinguishedName
        ...
        If ($dn) {
            ...
            foreach ($dc in $getdc) {
                ...
                Try {
                    ...
                    $aduser = Get-ADObject $dn -Server $dc -Properties lastlogon,whenCreated,Description,samAccountName,userAccountControl -ErrorAction Stop
                    ...
                    $resultlogon += New-Object -TypeName PSObject -Property ([ordered]@{
                            ...
                            'Enabled'        = if (($aduser.userAccountControl -band 0x2) -eq 2) { $false } else {$true}
        ...
        { Write-Error "Object '$object' not found. Check entered SAM Account Name." }
